...
(function(root, factory) {
 ...
 var keyMappings = {
  ...
  KeyNavigator.prototype = {
    ...
    enter: function($el, cellIndex, e, callBack){
    	$el.trigger('eventEnter',[$el]);
    	if(callBack){
    		callBack();
    	}
    },
    
    back: function($el, cellIndex, e, callBack){
    	$el.trigger('eventBack',[$el]);
    	if(callBack){
    		callBack();
    	}
    },
    
    g: function($el, cellIndex, e, callBack){
      $el.trigger('eventG',[$el]);
      if(callBack){
        callBack();
      }
    },
    
    doClose: function($el, cellIndex, e, callBack){
        $el.trigger('doClose',[$el]);
        if(callBack){
          callBack();
        }
      },
    
    pause: function($el, cellIndex){
    	$el.trigger('pause',[$el]);
    },
    
    stop: function($el, cellIndex){
    	$el.trigger('stop',[$el]);
    },
    
    play: function($el, cellIndex){
    	$el.trigger('play',[$el]);
    },
    
    rewind: function($el, cellIndex){
    	$el.trigger('rewind',[$el]);
    },
    
    fastfoward: function($el, cellIndex){
    	$el.trigger('fastfoward',[$el]);
    },
    ...
    down: function($el, cellIndex, e, fnCallback) {
      $el.trigger('down', [$el]);

      var colCells = this.cellTable.columns;
      var collPos = CellFactory.createFrom($el).pos.left;

      this.move({
        cellPosition: collPos,
        index: cellIndex.rowIndex + 1,
        cells: colCells,
        firstIndex: true
      });

      var cells = colCells[collPos],
	      cell = cells[cellIndex.rowIndex + 1];

      if (cell) {
		  if(this.hasEvent($el,'scrollCheckEvent')){
			  $el.trigger('scrollCheckEvent', {ele: cell.$el,
	   				direction:1 , cel: cellIndex.rowIndex + 1, callBack: fnCallback} );  
		  } else {fnCallback();}
	       
	  } else {
		  if(this.hasEvent($el,'isLastRowEvent')){
		  $el.trigger('isLastRowEvent', {ele: $el,
 				direction:1 , cel: cellIndex.rowIndex, callBack: fnCallback} );
		  } else {fnCallback();}
	  }
     
    },

    up: function($el, cellIndex, e, fnCallback) {
      $el.trigger('up', [$el]);
    ...
    left: function($el, cellIndex, e, fnCallback) {
      $el.trigger('left', [$el]);

      var rowCells = this.cellTable.rows;
      var cellPos = CellFactory.createFrom($el).pos.top;
      ...
      var cells = rowCells[cellPos],
          cell = cells[cellIndex.colIndex - 1];
      if (!cell) {
          //alert('ultimo Item');
           $el.trigger('noMoreItemsLeft', [$el]);
      }
      fnCallback();
    },

    right: function($el, cellIndex, e, fnCallback) {
      $el.trigger('right', [$el]);
      ...
      if (!cell) {
          //alert('ultimo Item');
           $el.trigger('noMoreItemsRight', [$el]);
      }
      fnCallback();
    },
    
    hasEvent : function (el, nameEvent) {
    	var events = $._data(el[0], 'events' );
    	var value = false;
    	$.each(events, function(i, ev){
    		if(ev[0].type===nameEvent){
    			value = true;
    			return false;
    		}
    	});
   ...
   // 	alert(e.which);
      ...
      var fnCallback = function(){
    	  that.blockKeys = false;
      };
      if(!this.blockKeys) {
	      if (navigationHandle) {
	    	  this.blockKeys = true;  
	    	  var funcc =  navigationHandle.apply(this, [$selected, cell, e, fnCallback]);
	    	 // this.blockKeys = false;
	    	  return funcc;
	      }
